...
MR/PR description templates live in [assets/](assets/): [feature-to-integration.md](assets/feature-to-integration.md) 提测单 · [integration-to-production.md](assets/integration-to-production.md) 发版单 · [hotfix-to-production.md](assets/hotfix-to-production.md) 事故单.
...
**Core rules on copy:** commit/PR message format is a hard gate — Conventional Commits, `type(scope): subject`, type from the standard enum, scope from repo convention; **language is flexible** (中文 / English), following the user's own phrasing/register, not translated into an alien voice ([voice.md](references/voice.md)).
...
4. EVERY commit message MUST be Conventional-Compliant (`type(scope): subject`) and non-obscured; language is free (中文/English) but the format is not.
...
6. **Draft MR/PR title + description** ([mr-pr.md](references/mr-pr.md) §4) — infer the intent (提测单 / 发版单 / 事故单) from the branches' roles, use its [assets/](assets/) template; from *all* commits vs baseline, not only HEAD.
...
| "Just regenerate the whole MR body from the commit list" | On a 发版单 (long-lived release MR) that destroys hand-curated prose/tables. Use incremental re-derivation. |
| "Every MR/PR gets the same description shape" | A 提测单, 发版单, and 事故单 have different readers — verify steps vs release notes+rollback vs root cause. Use the intent template. |
...
- [ ] MR/PR checked for this branch (incl. merged): existing → synced automatically (incremental for 发版单); none → asked before creating (or user declined)
